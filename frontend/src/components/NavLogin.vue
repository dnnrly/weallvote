  ...
  <div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-sm relative">
        ...
        <button @click="loginWithGoogle" class="w-full mt-3 bg-red-500 text-white py-2 rounded hover:bg-red-600">
          Sign in with Google
        </button>
        ...
        <button @click="isModalOpen = false" data-testid="modal-close-button"
...
const register = async () => {
  ...
  try {
    ...
    const token = await user.getIdToken();
    // 3. POST to backend /auth/register
    const res = await fetch('/auth/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!res.ok) {
      const msg = await res.text();
      throw new Error(`Backend rejected registration: ${msg}`);
    }
    isModalOpen.value = false;
  } catch (err: any) {
    error.value = err.message;
